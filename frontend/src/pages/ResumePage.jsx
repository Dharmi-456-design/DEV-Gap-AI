...
export default function ResumePage() {
  ...
  if (fetching) return <div className="p-6 flex items-center justify-center min-h-64"><div className="w-8 h-8 border-4 border-primary-500 border-t-transparent rounded-full animate-spin" /></div>;
    ...
    <div className="p-6 max-w-5xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-xl flex items-center justify-center shadow-lg shadow-primary-500/20">
            <RiFileTextLine className="text-xl text-white" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white">Resume Intelligence</h1>
            <p className="text-slate-500 text-sm">Deep AI analysis & job readiness evaluation</p>
          </div>
        </div>
        {resume && (
          <button 
            onClick={() => setRecruiterMode(!recruiterMode)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all border ${recruiterMode ? 'bg-red-500/10 text-red-400 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'bg-slate-900/40 text-slate-500 border-white/10 hover:border-white/20'}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div
            {...getRootProps()}
            className={`neon-glass-card p-8 text-center cursor-pointer border-2 border-dashed transition-all duration-300 ${
              isDragActive ? 'border-primary-500/60 bg-primary-500/5' : 'border-white/10 hover:border-primary-500/30 hover:bg-slate-800/2'
            }`}
          >
            <input {...getInputProps()} />
            {loading ? (
          ...
          {/* Current File Info */}
          {resume && (
            <div className="neon-glass-card p-5">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Active File</h3>
              <div className="flex items-center gap-3">
        ...
        {resume && (
          <div className="lg:col-span-2 space-y-6">
            ...
            {recruiterMode && (
              <div className={`neon-glass-card p-6 border-l-4 ${resume.recruiterVerdict?.hire ? 'bg-emerald-900/20 border-l-emerald-500' : 'bg-red-900/20 border-l-red-500'}`}>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="neon-glass-card p-5 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="neon-glass-card p-5 bg-red-900/5 border border-red-500/10">
                <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-3">
                  <RiPsychotherapyLine className="text-red-400" /> Reality Check
                </h3>
                <p className="text-sm text-slate-300 leading-relaxed bg-slate-900/60 p-3 rounded-lg border-l-2 border-l-red-500">
                  {resume.realityCheck}
                </p>
              </div>

              <div className="neon-glass-card p-5 bg-secondary-900/5 border border-secondary-500/10">
                <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-3">
                  <RiErrorWarningLine className="text-secondary-400" /> Project Issues Detected
                </h3>
                <ul className="space-y-2">
                  {resume.projectQuality?.issues?.map((issue, i) => (
                     <li key={i} className="text-xs text-slate-300 flex items-start gap-2">
                       <span className="text-secondary-500">•</span> {issue}
                     </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="neon-glass-card p-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="bg-red-500/5 p-4 rounded-xl border border-red-500/10">
                    ...
                    <p className="text-[10px] text-slate-500 mb-3">Missing core tools commonly required for target roles.</p>
            ...
            <div className="neon-glass-card p-6 bg-gradient-to-r from-slate-800 to-slate-900 border border-primary-500/20">
               <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                 <div className="flex-1">
                   <h3 className="text-md font-bold text-white flex items-center gap-2 mb-3">
                     <RiStarLine className="text-primary-400" /> Improve Your Resume
                   </h3>
                   <ul className="space-y-2">
                 ...
                 <div className="flex flex-col gap-3">
                   <button onClick={() => setShowGuide(true)} className="btn-primary flex items-center justify-center gap-2 w-full text-xs py-2 bg-slate-900/40 hover:bg-primary-500/10 hover:text-primary-400 border border-primary-500/30">
                     <RiBookOpenLine /> View Perfect Resume Guide
                   </button>
                   <Link to="/mentor?action=fix_resume" className="btn-primary whitespace-nowrap flex items-center justify-center gap-2">
      ...
      {showGuide && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900/60 border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-white/10 bg-slate-900/60">
              <h2 className="text-xl font-black text-white flex items-center gap-2">
                <RiBookOpenLine className="text-primary-400" /> The Perfect Developer Resume
              </h2>
              <button onClick={() => setShowGuide(false)} className="text-slate-500 hover:text-white transition-colors bg-slate-900/40 p-2 rounded-lg">
                <RiCloseLine size={24} />
              </button>
            </div>
            <div className="p-6 overflow-y-auto space-y-6 text-slate-300">
              ...
              <section>
                ...
                <div className="bg-slate-950 p-4 rounded-lg border-l-2 border-primary-500 text-sm italic">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><strong className="text-secondary-400">Tools:</strong><br/>VS Code, Figma, Chrome DevTools</div>
                </div>
              </section>
              ...
              <section>
                ...
                <ul className="list-disc pl-5 text-sm space-y-1 text-slate-500 mb-3">
                  <li>Project Name & Short description</li>
                  <li>Tech stack used</li>
                  <li>Live Demo link + GitHub repo link</li>
                </ul>
                <div className="bg-slate-950 p-4 rounded-lg border-l-2 border-emerald-500 text-sm">
                  <strong>GitHub Portfolio Analyzer</strong><br/>
                  <span className="text-slate-500">Built a tool to analyze GitHub profiles and display insights. Used React, API integration, and modern UI design.</span><br/>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <section>
                  ...
                  <p className="text-sm text-slate-500">Degree, College name, Graduation Year.</p>
                </section>
                <section>
                  <h3 className="text-white font-bold mb-2">🔹 6. Experience</h3>
                  <p className="text-sm text-slate-500">Internship / Freelance. If no experience → expand "Projects".</p>
                </section>
              </div>

              <section className="bg-primary-500/10 p-5 rounded-xl border border-primary-500/20">
                <h3 className="text-primary-400 font-black text-lg mb-3">🚀 Pro Tips (VERY IMPORTANT)</h3>
                <ul className="space-y-2 text-sm">
                  <li className="flex gap-2"><RiCheckLine className="text-primary-500 shrink-0 mt-0.5"/> <strong>Keep it 1 page only.</strong></li>
                  <li className="flex gap-2"><RiCheckLine className="text-primary-500 shrink-0 mt-0.5"/> <strong>Simple design.</strong> No heavy colors or weird formats that break ATS.</li>
                  <li className="flex gap-2"><RiCheckLine className="text-primary-500 shrink-0 mt-0.5"/> <strong>Show, don't tell.</strong> Don’t write “hardworking, passionate” → prove it via complex projects.</li>
                  <li className="flex gap-2"><RiCheckLine className="text-primary-500 shrink-0 mt-0.5"/> <strong>Action Words.</strong> Start points with Built, Developed, Designed.</li>
                </ul>
              </section>
