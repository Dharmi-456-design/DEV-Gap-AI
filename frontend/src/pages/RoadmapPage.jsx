      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-xl flex items-center justify-center shadow-lg shadow-primary-500/20">
            <RiRoadMapLine className="text-xl text-white" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white">Learning Roadmap</h1>
            <p className="text-slate-500 text-sm">Personalized step-by-step career path with videos</p>
          </div>
        </div>
        <button onClick={handleGenerate} disabled={generating} className="btn-primary flex items-center gap-2 text-sm">
          {generating ? <div className="w-4 h-4 border-2 border-slate-900/50 border-t-slate-900 rounded-full animate-spin" /> : '⚡'} Generate
        </button>
      </div>

      {!roadmap ? (
        <div className="neon-glass-card p-16 text-center">
          <RiRoadMapLine className="text-7xl text-slate-300 mx-auto mb-5" />
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={handleGenerate} disabled={generating} className="btn-primary flex items-center gap-2">
              {generating ? <div className="w-4 h-4 border-2 border-slate-900/50 border-t-slate-900 rounded-full animate-spin" /> : '🚀'} Generate Roadmap
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-8 max-w-4xl mx-auto">
          {/* Mentor Greeting */}
          <div className="relative p-6 bg-slate-900/60 border border-purple-500/30 rounded-2xl animate-slide-up shadow-xl shadow-purple-500/10">
          ...
          <div className="flex items-center gap-4 py-4">
            <div className="flex-1 h-px bg-slate-900/40/5"></div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest text-center">Your Brutal Action Plan</p>
            <div className="flex-1 h-px bg-slate-900/40/5"></div>
          </div>
          ...
          <div className="space-y-8">
            {roadmap.phases?.map((phase, i) => (
              <div key={i} className="neon-glass-card overflow-hidden animate-slide-up relative" style={{ animationDelay: `${i * 100}ms` }}>
                
                {/* Mentor's Direct Instruction for the phase */}
                <div className="p-6 bg-gradient-to-r from-purple-900/40 to-slate-900 border-b border-white/5">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mb-3 font-semibold tracking-wide">DURATION: {phase.duration.toUpperCase()}</p>
                      
                      {phase.mentorMessage && (
                        <div className="p-4 bg-slate-950/50 border-l-4 border-purple-500 rounded-r-xl">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-xs font-bold text-primary-400 uppercase tracking-wider mb-2">Mandatory Tasks</p>
                        ...
                        <div
                          ...
                          className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all duration-200 ${
                            task.completed
                              ...
                              : 'border-white/5 bg-slate-900/40/5 hover:border-primary-500/20 hover:bg-slate-800/5'
                          }`}
                        >
                          ...
                          <div className="flex-1">
                            <p className={`text-sm font-medium ${task.completed ? 'text-slate-500 line-through' : 'text-white'}`}>{task.task}</p>
                          </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {phase.practiceLinks?.map((link, vi) => (
                          <div key={vi} className="flex flex-col p-4 bg-slate-900/60 border border-white/10 rounded-xl hover:border-blue-500/40 hover:shadow-[0_0_15px_rgba(59,130,246,0.1)] hover:-translate-y-0.5 transition-all duration-300">
                            <div className="flex items-start gap-4 mb-3">
                              <div className="p-2.5 bg-slate-950 border border-white/5 shadow-inner rounded-xl shrink-0">
                              ...
                              <div>
                                ...
                                <p className="text-slate-500 text-xs mt-1 leading-relaxed pr-2">{link.description || 'Practice your coding logic instantly.'}</p>
                              </div>
                            </div>
                        ...
                        {(!phase.practiceLinks || phase.practiceLinks.length === 0) && (
                          <p className="text-xs text-slate-400 italic p-4 bg-slate-900/60 rounded-xl border border-white/5">No specific platforms listed.</p>
                        )}
                      </div>

                      {/* Dynamic Hidden Reveal Container for Standard Platforms */}
                      {revealPlatforms[phase.phase] && (
                        <div className="mt-4 p-5 bg-gradient-to-br from-slate-800 to-slate-900 border border-primary-500/20 rounded-2xl animate-slide-up shadow-xl shadow-primary-500/5">
                          ...
                          <div className="flex flex-col gap-3">
                            <a href="https://www.sololearn.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 bg-slate-950/50 rounded-xl border border-white/5 hover:border-blue-500/50 hover:bg-blue-500/10 transition-all group">
                              ...
                              <div>
                                ...
                                <p className="text-slate-500 text-xs">Beginner-friendly coding practice</p>
                              </div>
                            </a>
                            <a href="https://www.hackerrank.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 bg-slate-950/50 rounded-xl border border-white/5 hover:border-[#00EA64]/50 hover:bg-[#00EA64]/10 transition-all group">
                              ...
                              <div>
                                ...
                                <p className="text-slate-500 text-xs">Skill testing and coding challenges</p>
                              </div>
                            </a>
                            <a href="https://leetcode.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 bg-slate-950/50 rounded-xl border border-white/5 hover:border-[#FFA116]/50 hover:bg-[#FFA116]/10 transition-all group">
                              ...
                              <div>
                                ...
                                <p className="text-slate-500 text-xs">Advanced problem-solving and interview preparation</p>
                              </div>
                            </a>
                          </div>
                        </div>
                    ...
                    <div className="space-y-3">
                      <button 
                        ...
                        Test Your Knowledge 
                        <RiArrowRightSLine className={`text-lg transition-transform duration-300 ${revealPlatforms[phase.phase] ? 'rotate-90 text-primary-400' : 'group-hover:translate-x-1'}`} />
                      </button>
                      <div className="space-y-2">
                        {phase.interviewQuestions?.map((q, qi) => (
                          <a key={qi} href={q.url} target="_blank" rel="noopener noreferrer"
                             className="block p-3 bg-slate-900/60 border border-white/5 rounded-xl border-l-2 border-l-pink-500/50 hover:bg-pink-500/10 hover:border-pink-500/30 transition-all duration-200 group">
