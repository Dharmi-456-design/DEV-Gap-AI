  ...
  if (!data) {
    return (
      <div className="p-6 max-w-4xl mx-auto animate-fade-in flex flex-col items-center justify-center min-h-[80vh]">
        ...
        <p className="text-slate-400 text-center max-w-xl mb-10 leading-relaxed">
    ...
    <div className="p-6 max-w-4xl mx-auto animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 mt-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Target Role: <strong className="text-white bg-white/5 px-2 py-0.5 rounded ml-1">{data.target_role}</strong></p>
        </div>
        <button onClick={handleGenerate} className="px-5 py-2.5 bg-obsidian-800 border border-white/10 hover:border-purple-500/50 text-slate-300 hover:text-purple-400 rounded-xl transition-all text-sm font-bold flex items-center justify-center gap-2">
           Re-evaluate Plan
        </button>
      </div>
      ...
      {data.show_reality_warning && (
        <div className="mb-8 p-5 rounded-2xl bg-gradient-to-r from-red-950/80 to-obsidian-900 border-l-4 border-l-red-500 border border-red-500/20 shadow-xl shadow-red-500/5 items-center flex gap-4 animate-slide-up">
      ...
      {!data.show_reality_warning && (
         <div className="mb-8 p-5 rounded-2xl bg-gradient-to-r from-emerald-950/80 to-obsidian-900 border-l-4 border-l-emerald-500 border border-emerald-500/20 shadow-xl shadow-emerald-500/5 items-center flex gap-4 animate-slide-up">
      ...
      {/* Mentor Intro */}
      <div className="mb-10 px-6 py-5 bg-obsidian-800 border border-purple-500/30 rounded-2xl relative">
         <div className="absolute -top-4 -left-3 w-10 h-10 bg-gradient-to-br from-purple-500 to-blue-500 rounded-xl flex items-center justify-center text-white shadow-lg border-2 border-obsidian-900">
            <RiPsychotherapyLine className="text-lg" />
         </div>
         <p className="text-slate-100 text-[15px] italic leading-relaxed ml-4">
            "{data.mentor_intro}"
         </p>
      </div>
      ...
      <div className="space-y-4">
        {data.phases.map((phase, i) => {
          ...
          return (
            <div 
              key={i} 
              className="glass-card overflow-hidden transition-all duration-300 border border-white/5 hover:border-purple-500/30 bg-obsidian-900"
            >
              <button 
                onClick={() => togglePhase(i)} 
                className="w-full text-left p-5 flex items-center justify-between hover:bg-white/[0.02] transition-colors focus:outline-none"
              >
                 ...
                 {isExpanded ? <RiArrowUpSLine className="text-slate-400 text-xl" /> : <RiArrowDownSLine className="text-slate-400 text-xl" />}
              </button>

              {isExpanded && (
                <div className="p-6 pt-0 border-t border-white/5 bg-obsidian-950/40">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                         ...
                         <div className="space-y-2">
                           {phase.tasks.map((task, idx) => (
                              <div key={idx} className="flex gap-3 p-3 bg-obsidian-900 border border-white/5 rounded-xl hover:border-white/20 transition-all">
                                 <RiCheckDoubleLine className="text-slate-500 mt-0.5" />
                      ...
                      <div>
                         ...
                         <p className="text-[10px] uppercase font-bold text-slate-500 mb-2 pl-1">Practice & Prove it</p>
                         <div className="grid grid-cols-2 gap-2">
                            {phase.resources.practice_links.map((link, idx) => (
                               <a 
                                 ...
                                 href={link.url} 
                                 target="_blank" rel="noopener noreferrer"
                                 className="flex items-center justify-between p-2.5 bg-obsidian-800 border border-white/10 rounded-xl hover:border-blue-500/50 hover:bg-blue-500/10 transition-all group"
                               >
                                 <span className="text-xs font-semibold text-slate-300 group-hover:text-blue-400">{link.name}</span>
                                 <RiExternalLinkLine className="text-slate-500 text-xs" />
                               </a>
                            ))}
                         </div>
                      </div>
