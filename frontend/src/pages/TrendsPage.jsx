...
function MockTestGeneratorCard({ missingSkills }) {
  ...
  return (
    <div className="glass-card-hover p-6 md:p-8 bg-gradient-to-br from-obsidian-800 to-obsidian-900 border-transparent hover:border-purple-500/50 relative overflow-hidden group mb-5">
      {/* Animated gradient glow border base */}
      <div className="absolute inset-0 bg-gradient-to-r from-purple-600/10 to-orange-500/10 opacity-50 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-8 items-start lg:items-center">
        ...
        <div className="flex-1">
          ...
          <div className="mb-6 space-y-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Detected Missing Skills</h3>
            {missingSkills?.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {skillsToTest.map(skill => (
                  <span key={skill} className="px-3 py-1.5 bg-obsidian-700 border border-white/10 rounded-xl text-sm font-medium text-slate-300 shadow-sm group-hover:border-purple-500/30 group-hover:text-purple-300 transition-all cursor-default">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-slate-400">
            <div className="flex items-center gap-1.5 px-3 py-2 bg-obsidian-950/50 rounded-lg border border-white/5">
              <RiTimeLine className="text-gold-400 text-lg" />
        ...
        <div className="w-full lg:w-72 mt-2 lg:mt-0">
          <button 
            ...
            className={`w-full py-4 rounded-xl font-black text-sm shadow-[0_0_20px_rgba(34,211,238,0.3)] transition-all duration-300 flex items-center justify-center gap-2 ${
              missingSkills?.length === 0 
                ? 'bg-slate-800 text-slate-500 shadow-none cursor-not-allowed opacity-70' 
          ...
          <div className="mt-6 p-4 bg-obsidian-950/50 rounded-xl border border-white/5">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="h-1 bg-obsidian-800 rounded-full w-full">
    ...
    <div className="p-6 max-w-5xl mx-auto animate-fade-in">
      <div className="flex items-center gap-3 mb-8">
        <div className="w-10 h-10 bg-gradient-to-br from-gold-500 to-orange-500 rounded-xl flex items-center justify-center shadow-lg shadow-gold-500/20">
          <RiLineChartLine className="text-xl text-obsidian-900" />
        </div>
        <div>
          <h1 className="text-2xl font-black text-white">Future Job Trends</h1>
          <p className="text-slate-400 text-sm">Trending skills, emerging roles & dying careers</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-400 live-pulse" />
          <span className="text-xs text-slate-400">Live 2024 Data</span>
        </div>
      </div>
      ...
      <div className="glass-card p-6 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {trends?.trendingSkills?.map((item, i) => (
            <div key={i} className="flex items-center gap-4 p-4 bg-obsidian-700/50 border border-white/5 rounded-xl hover:border-gold-500/20 transition-all duration-200 group">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-white font-semibold text-sm group-hover:text-gold-400 transition-colors">{item.skill}</p>
                  <span className="text-emerald-400 text-xs font-bold">{item.growth}</span>
                </div>
                <p className="text-slate-400 text-xs">{item.salary}</p>
              </div>
              <div className="text-right">
                <span className={`px-2 py-1 rounded-lg text-xs font-bold ${diffColor[item.difficulty] || 'text-slate-400 bg-slate-500/10'}`}>
                  {item.difficulty}
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="glass-card p-6">
          ...
          <div className="space-y-3">
            {trends?.topEmergingRoles?.map((role, i) => (
              <div key={i} className="flex items-center gap-3 p-3 bg-obsidian-700/40 rounded-xl">
                <div className="w-7 h-7 bg-gradient-to-br from-gold-500 to-orange-500 rounded-full flex items-center justify-center text-obsidian-900 text-xs font-black flex-shrink-0">
                  {i + 1}
                </div>
        ...
        <div className="glass-card p-6">
          ...
          <p className="text-slate-400 text-xs mb-4">Roles being automated or losing demand</p>
          <div className="space-y-3">
          ...
          <div className="mt-4 p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl">
            ...
            <p className="text-slate-400 text-xs">Automation is replacing repetitive, low-skill roles. Focus on high-value, creative, and system-level skills.</p>
          </div>
        </div>
      </div>
      ...
      {/* Summary Banner */}
      <div className="glass-card p-5 bg-gradient-to-r from-gold-500/10 to-orange-500/10 border border-gold-500/20">
