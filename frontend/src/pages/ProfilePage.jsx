...
function CareerJournal() {
  ...
  const moodColors = {
    motivated: 'text-secondary-400 bg-secondary-500/10 border-secondary-500/20',
    focused: 'text-blue-400 bg-blue-500/10 border-blue-500/20',
    tired: 'text-slate-500 bg-slate-900/600/10 border-slate-500/20',
    ...
    <div className="neon-glass-card p-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Track your daily progress &amp; thoughts</p>
        </div>
      ...
      <div className="mb-5 space-y-3">
        <div className="flex gap-2 flex-wrap mb-2">
          {moods.map(m => (
            <button key={m.key} onClick={() => setMood(m.key)}
              className={`px-3 py-1 rounded-full text-xs font-bold border transition-all ${mood === m.key ? moodColors[m.key] : 'border-white/5 text-slate-400 hover:border-white/10'}`}>
              {m.emoji} {m.label}
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-slate-900/40/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-purple-500/50 transition-all resize-none text-sm"
          />
      ...
      {entries.length === 0 ? (
        <div className="text-center py-8 border border-dashed border-white/5 rounded-xl">
          ...
          <p className="text-slate-400 text-sm">No entries yet. Start journaling your progress!</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
          {entries.map(entry => (
            <div key={entry.id} className="p-4 bg-slate-900/40/5 border border-white/5 rounded-xl group">
    ...
    <div className="p-6 max-w-5xl mx-auto animate-fade-in space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage your identity &amp; track your career journey</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          {/* Avatar Card */}
          <div className="neon-glass-card p-6 text-center">
            <div className="w-24 h-24 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-3xl flex items-center justify-center text-white text-5xl font-black shadow-2xl shadow-primary-500/30 mx-auto mb-4">
              {user?.name?.[0]?.toUpperCase()}
            </div>
            <h2 className="text-white font-black text-xl">{user?.name}</h2>
            <p className="text-slate-500 text-sm mt-1">{user?.email}</p>
            {form.targetRole && (
              <span className="inline-block mt-2 px-3 py-1 bg-primary-500/10 text-primary-400 border border-primary-500/20 rounded-full text-xs font-bold">
                {form.targetRole}
              </span>
            )}
            {form.location && (
              <p className="text-slate-400 text-xs mt-2 flex items-center justify-center gap-1">
                <RiMapPinLine className="text-slate-500" /> {form.location}
              </p>
            )}
          </div>
          ...
          <div className="neon-glass-card p-5">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Platform Progress</h3>
            <div className="space-y-3">
              {/* DevGap Score */}
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-slate-500 flex items-center gap-1"><RiSparklingLine className="text-primary-400" />DevGap Score</span>
                  <span className="text-primary-400 font-bold">{stats.devGapScore}/100</span>
                </div>
                <div className="h-2 bg-slate-900/40/10 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-slate-500 flex items-center gap-1"><RiCheckLine className="text-emerald-400" />Modules Done</span>
            ...
            <div className="mt-4 space-y-2">
              {[
                ...
                <Link key={label} to={path}
                  className={`flex items-center gap-2 p-2.5 rounded-xl border text-xs font-medium transition-all ${done ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-400' : 'border-white/5 text-slate-400 hover:border-white/10 hover:text-slate-300'}`}>
          ...
          <div className="neon-glass-card p-5">
            ...
            <div className="space-y-1">
              {[
                ...
                { label: 'Job Trends', path: '/trends', icon: RiFireLine, color: 'text-secondary-400' },
              ].map(({ label, path, icon: Icon, color }) => (
                <Link key={path} to={path}
                  className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-800/5 transition-colors group text-sm text-slate-500 hover:text-white">
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Edit Form */}
          <form onSubmit={handleSave} className="neon-glass-card p-6 space-y-4">
            <h3 className="font-bold text-white text-base flex items-center gap-2 mb-2">
              <RiPencilLine className="text-primary-400" /> Edit Details
            </h3>
            ...
            <button type="submit" disabled={saving} className="btn-primary w-full flex items-center justify-center gap-2 mt-2">
              {saving ? (
                <div className="w-5 h-5 border-2 border-slate-900/50 border-t-slate-900 rounded-full animate-spin" />
              ) : saved ? (
