...
export default function InterviewPrepPlatform() {
  ...
  if (setupMode) {
    return (
      <div className="min-h-screen bg-obsidian-950 p-6 flex flex-col items-center justify-center relative overflow-hidden">
        {/* Background elements */}
        ...
        <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8 z-10">
           {/* Left Pane - Controls */}
           <div className="lg:col-span-2 glass-card p-8 md:p-10 border-white/5 shadow-2xl relative overflow-hidden">
              ...
              <p className="text-slate-400 mb-8">Professional Answer-Based Learning & Mock Interview Simulator</p>

              <div className="space-y-6 mb-8">
                 <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Select Category</h3>
                      ...
                      {["All", "Data Structures", "Web Development", "HR Interview", "Database (SQL)", "System Design Basics", "Algorithms", "Behavioral"].map(cat => (
                        <button key={cat} onClick={() => setSelectedCategory(cat)} className={`py-2 px-3 rounded-xl border text-xs font-bold transition-all text-center ${
                          selectedCategory === cat 
                            ...
                            : 'bg-obsidian-800 border-white/5 text-slate-400 hover:border-white/20'
                        }`}>
                          {cat}
                        </button>
                      ))}
                 ...
                 <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Select Difficulty</h3>
           ...
           {/* Right Pane - Metrics */}
           <div className="glass-card p-6 h-full border-white/5 flex flex-col">
              <h3 className="section-title text-sm mb-4">Performance Dashboard</h3>
              
              <div className="space-y-4 mb-8">
                 <div className="bg-obsidian-900 p-4 rounded-xl border border-white/5 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">QUESTIONS PRACTICED</p>
                 ...
                 <div className="bg-obsidian-900 p-4 rounded-xl border border-purple-500/20 flex items-center justify-between shadow-[0_0_15px_rgba(168,85,247,0.1)]">
                    <div>
                 ...
                 <div className="bg-obsidian-900 p-4 rounded-xl border border-emerald-500/20 flex items-center justify-between">
                    <div>
              ...
              <div className="mt-auto">
                 <button onClick={() => navigate('/trends')} className="w-full py-3 bg-obsidian-800 text-white font-bold rounded-xl hover:bg-obsidian-700 transition flex justify-center items-center gap-2 text-sm">
  ...
  return (
    <div className="min-h-screen bg-obsidian-950 font-sans text-slate-300 flex flex-col overflow-y-auto global-scrollbar">
      {/* Top Navbar */}
      <div className="h-16 border-b border-white/10 bg-obsidian-900 flex items-center justify-between px-6 flex-shrink-0 sticky top-0 z-50">
         <div className="flex items-center gap-4">
            <button onClick={() => setSetupMode(true)} className="text-slate-400 hover:text-white transition flex items-center gap-1 text-sm font-bold bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-slate-500 font-medium">Difficulty: <span className="text-cyan-400">{question.difficulty}</span></span>
            </div>
         </div>
         <div className="flex items-center gap-3">
             <button className="text-xs font-bold text-slate-400 hover:text-white border border-white/5 px-3 py-1.5 rounded-lg flex items-center gap-2">
                 <RiBookmarkLine /> Save Note
             </button>
             <button onClick={startPractice} className="text-xs font-bold text-obsidian-900 bg-cyan-400 hover:bg-cyan-300 px-4 py-1.5 rounded-lg flex items-center gap-2 transition-colors">
      ...
      <div className="max-w-6xl w-full mx-auto p-6 md:p-8 flex flex-col lg:flex-row gap-8">
         ...
         <div className="flex-1 flex flex-col gap-6">
            <div className="glass-card p-6 md:p-8 border-cyan-500/20 shadow-[0_4px_30px_rgba(34,211,238,0.05)] relative overflow-hidden">
               <div className="absolute top-0 right-0 p-4">
                  <span className="px-3 py-1 bg-obsidian-900 border border-white/10 rounded-full text-xs font-bold text-slate-400 flex items-center gap-1">
               ...
               <div className="flex flex-wrap gap-2 mb-6">
                  <span className="text-[10px] uppercase font-bold text-slate-500 bg-obsidian-900 px-2 py-1 rounded border border-white/5">Key Topics:</span>
               ...
               <div className="p-4 bg-obsidian-900 border border-white/5 rounded-xl border-l-4 border-l-gold-500 mb-2">
                  <p className="text-xs uppercase font-bold text-gold-400 mb-1">Expected Depth</p>
                    ...
                    {question.hints.map((hint, i) => (
                      <div key={i}>
                          {hintsRevealed >= i + 1 ? (
                            ...
                            hintsRevealed === i && (
                              <button onClick={() => setHintsRevealed(r => r + 1)} className="w-full py-3 bg-obsidian-900 hover:bg-obsidian-800 text-slate-500 hover:text-slate-300 text-xs rounded-xl font-bold transition flex items-center justify-center gap-2 border border-dashed border-white/10 hover:border-amber-500/30">
            ...
            <div className="glass-card p-6 border-white/5 flex flex-col pt-6">
                <div className="flex items-center justify-between mb-4">
                ...
                <textarea 
                  ...
                  className="w-full h-40 bg-obsidian-900 border border-white/10 rounded-xl p-4 text-sm text-slate-300 focus:outline-none focus:border-cyan-500/50 resize-none leading-relaxed mb-4"
                />

                <div className="flex items-center gap-3">
                  <button className="h-12 w-12 rounded-xl bg-obsidian-900 hover:bg-obsidian-800 border border-white/10 flex items-center justify-center text-slate-400 hover:text-emerald-400 transition-colors tooltip-trigger relative group">
                    <RiMicLine className="text-xl" />
                    <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-obsidian-800 text-[10px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap">Voice Mode (Coming Soon)</span>
                  </button>
                  <button 
         ...
         <div className="w-full lg:w-[450px] flex flex-col gap-6">
            ...
            {!showAnswer ? (
                <div className="glass-card p-8 border-dashed border-white/10 text-center h-full flex flex-col items-center justify-center min-h-[400px]">
                  <RiBookOpenLine className="text-6xl text-slate-700 mx-auto mb-4" />
                ...
                <div className="flex flex-col gap-6">
                   ...
                   {evaluation && (
                      <div className="glass-card p-6 border-emerald-500/30 bg-emerald-500/5 animate-slide-up shadow-[0_0_20px_rgba(16,185,129,0.1)]">
                         ...
                         <div className="grid grid-cols-2 gap-3 mb-5">
                            <div className="p-3 bg-obsidian-900 border border-white/5 rounded-lg text-center">
                              <p className="text-2xl font-black text-white">{evaluation.conceptAccuracy}%</p>
                              <p className="text-[10px] text-slate-500 uppercase font-bold">Concept Accuracy</p>
                            </div>
                            <div className="p-3 bg-obsidian-900 border border-white/5 rounded-lg text-center">
                              <p className="text-2xl font-black text-cyan-400">{evaluation.confidenceScore}%</p>
                              <p className="text-[10px] text-slate-500 uppercase font-bold">Confidence Spec</p>
                            </div>
                         </div>
                   ...
                   {/* Optimal Answer View */}
                   <div className="glass-card p-6 border-indigo-500/20 bg-gradient-to-b from-indigo-500/5 to-transparent relative animate-fade-in shadow-[0_0_40px_rgba(99,102,241,0.05)]">
                      ...
                      <p className="text-sm text-slate-300 leading-relaxed mb-6 p-4 bg-obsidian-900 border border-white/5 rounded-xl border-l-2 border-l-indigo-400">
                         "{question.bestAnswer}"
                      </p>

                      <h3 className="text-xs uppercase font-bold text-slate-500 mb-2">Simple Explanation</h3>
                      <p className="text-sm text-slate-400 mb-6 italic border-l-2 border-white/10 pl-3">
                      ...
                      <div>
                         <h3 className="text-xs uppercase font-bold text-slate-500 mb-3 border-b border-white/10 pb-2">Possible Follow-Up Questions</h3>
